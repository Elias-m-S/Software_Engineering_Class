...
\begin{enumerate}
  ...
  \item \textbf{[P2 – Architektur]}
    \textbf{\ac{CAN}-Datenklasse ausdetaillieren.} \\
    Die Klasse für \ac{CAN}-Pakete (Struktur: ID, Payload, Timestamp) in einem
    eigenen Klassendiagramm modellieren und in Abschnitt~\ref{sec:klassendiagramme}
    einbinden.
    \todo{\ac{CAN}-Datenklasse modellieren und Diagramm einbinden.}

  \item \textbf{[P3 – Qualität]}
    \textbf{Latenzzahlen ergänzen.} \\
    Typische \ac{CAN}-Zykluszeit und \ac{LAN}-Latenz aus Systemtests eintragen
...
\begin{itemize}
  ...
  \item \assumption{\ac{CAN}-Zykluszeit $< 5\,\text{ms}$; zu messen.}
  \item \assumption{Distanzschwelle für Rot-Zone: $\leq 15\,\mathrm{cm}$.}
  \item \assumption{\ac{CAN}-Datenklasse kapselt ID, Payload und Timestamp.}
