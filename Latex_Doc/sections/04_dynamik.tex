...
\begin{enumerate}
  ...
  \item Über LAN $\to$ \texttt{MasterLanLink} $\to$ \texttt{MasterCanGateway}
        wird ein CAN-Paket generiert.
  \item Das CAN-Paket wird vom \texttt{EnvironmentControl}-Interface auf dem Slave
  ...
  \item Die Status-LED wechselt auf Rot.
\end{enumerate}
...
Beim Systemstart werden alle Komponenten initialisiert.
...
Nach einem Emergency Stop ist eine manuelle Freigabe erforderlich.
...
wird der Freigabe-Request \textbf{abgelehnt}.
...
Fehlen gültige Sensordaten (z.\,B.\ LiDAR-Ausfall, CAN-Timeout),
greift das \textbf{Fail-Safe-Prinzip}:

\begin{enumerate}
  \item \texttt{EnvironmentControl} detektiert fehlende/ungültige CAN-Daten.
...
Bei einem \texttt{ShutdownRequest}:
\begin{enumerate}
