...
\begin{enumerate}
  ...
  \item Das \ac{CAN}-Paket wird vom \texttt{EnvironmentControl Interface}-Interface auf dem Slave
  ...
  \item Der Emergency Stop wechselt in den Zustand \texttt{EmergencyStop},
        ...
        und löst einen \texttt{Logging.log(ERROR, "Safety zone triggered")}-Aufruf aus.
...
\begin{enumerate}
  ...
  \item \texttt{EmergencyStop} wechselt von \texttt{Initialization}
        nach \texttt{SoftStop}.
  \item \texttt{Logging} registriert \texttt{INFO: System startup complete}.
...
\begin{enumerate}
  ...
  \item \texttt{Logging.log(INFO, "System released, resuming normal operation")}.
\end{enumerate}
...
\texttt{Logging} protokolliert \texttt{WARNING: Release rejected – condition not met}.
...
\begin{enumerate}
  \item \texttt{EnvironmentControl Interface} detektiert fehlende/ungültige \ac{CAN}-Daten.
  ...
  \item \texttt{Logging.log(ERROR, "Sensor data missing – soft stop engaged")}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \texttt{Logging.log(INFO, "Shutdown initiated")}.
